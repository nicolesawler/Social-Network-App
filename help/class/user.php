...
class User{
    ...
    */
    public function login($email,$password){
        if(is_null($this->pdo)){
            ...
            $pdo = $this->pdo;
            $stmt = $pdo->prepare('SELECT user_id, first, last, user_email, user_pass, wrong_logins, user_role FROM users WHERE user_email = ? and confirmed = 1 limit 1');
            $stmt->execute([$email]);
            $user = $stmt->fetch();
    ...
    */
    public function registration($email,$fname,$lname,$pass){
        ...
        $stmt = $pdo->prepare('INSERT INTO users (first, last, user_email, user_pass, confirm_code) VALUES (?, ?, ?, ?, ?)');
    ...
    */
    public function listUsers(){
        if(is_null($this->pdo)){
            ...
            $pdo = $this->pdo;
            $stmt = $pdo->prepare('SELECT user_id, first, last, user_email FROM users WHERE confirmed = 1');
            $stmt->execute();
